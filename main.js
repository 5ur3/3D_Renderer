...
plane5.scale.y = 0.01;
plane5.enabled = 0;
plane5.shader.color = new Vector(0.2, 0.2, 0.2);
let ball5 = new Object3D();
ball5.scale.multiply(0.5);
ball5.position.y = 2;
ball5.mesh = Mesh.sphere;
ball5.enabled = 0;
ball5.shader.color = new Vector(0.8, 0.8, 0.2);
let ball5_speed = 0;
...
function update() {
	...
	for (let i = 0; i < balls5.length; i++) {
		...
		balls5[i].shader.color = new Vector(Math.sin((new Date).getTime() / 1000 + Math.PI * 6 / balls5.length * i) + 1,
						 			 Math.cos((new Date).getTime() / 1000 + Math.PI * 6 / balls5.length * i) + 1, 
						 			 1);
	}
	ball5_speed -= 0.0098;
